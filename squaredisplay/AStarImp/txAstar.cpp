...
void txAstar::CalculatePath()
{
	...
	accclose.reserve(closenodes.size());

	for (std::list<txNode*>::iterator it = closenodes.begin(); it!=closenodes.end(); it++)
	{
		accclose.push_back((*it)->i);
		accclose.push_back((*it)->j);
	}
...
txNode *txAstar::MinNodesInOpennodes()
{
	...
	// This is case to fix the bug 
	// that the maxlen is not only vertical len + horizental len
	const int pathtwistratio = 10;
	int min = maxlen*pathtwistratio;
	txNode *rtn=NULL;
